...
| SDRAM | `0xD0000000`, 16 MB chip; 8 MB mapped (16-bit FMC, 12×8×4) |
...
The part is **16 MB**. The 16-bit FMC stays 12 row × 8 col × 4 banks, so this firmware maps **8 MB**. Switching to 9 column bits remaps the AHB window and the LTDC UI is garbage. Framebuffers and viewers stay in the 8 MB window. RGB565 is the default pixel format (480×272×2 = 261 120 bytes per full buffer).
...
First bundled module: **Brick**. The Game app is a **library**: Brick, then built-in CHIP-8 carts (Demo, Snek, Super Pong, Br8kout), then files from `/user/game` (`*.ch8` / `*.c8`). The first eMMC-loadable core is **CHIP-8** (`game_module_t.load`). Extra cores register with `game_module_by_id` and an extension table; do not fork the app. No libretro / NES / GB in this generation.
...
FatFs itself is not reentrant (`FF_FS_REENTRANT=0`). `vfs.c` takes one OSAL mutex around volume objects and fd/dir slots so `ui` and `zb` can share `/user`. Path helpers stay lock-free. `znp` does not call VFS. The mutex is created after the scheduler starts; boot VFS on `main` is single-threaded. Creating a FreeRTOS mutex before `vTaskStartScheduler()` masks SysTick on Cortex-M.
