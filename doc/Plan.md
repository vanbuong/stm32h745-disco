...
A local game is optional (P2) and must use the same app contract as every other app.
...
- Dummy apps that push/pop screens.
...
- Settings app (brightness, volume, IP).
- HIL pack for the requirement matrix.
- **Exit:** RTM P0/P1 rows green; 8-hour soak (UI + audio + explorer) without leak or deadlock.
...
| Scope (game, Wi-Fi, NTP) | P2; do not block explorer/viewers |
...
| M5 (Sprint 10) | Full shell, status, soak |
