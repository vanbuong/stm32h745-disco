...
| FMC SDRAM bank 2 | M7 | 16 MB chip, 8 MB mapped (12×8×4), `0xD0000000` | `sdram.c` |
...
IS42S32800G (or 16-bit equivalent), **16 MB** die. 16-bit FMC stays 12 row × **8** col × 4 banks → **8 MB** at `0xD0000000`. 9 col remaps the AHB window and the LTDC framebuffer is garbage. MPU and `memtest` walk the 8 MB window only.
