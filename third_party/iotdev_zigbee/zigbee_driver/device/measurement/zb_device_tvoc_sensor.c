...
static const s_zb_attr_read_t s_tvoc_sensor_read_attrs[] = {
    { ZCL_CLUSTER_ID_MS_TVOC_MEASUREMENT, 1, { ATTRID_CONCENTRATION_MEASUREMENT_MEASURED_VALUE } },
};

static void
tvoc_sensor_init(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    s_zb_device_tvoc_sensor_ctx_t *ctx = (s_zb_device_tvoc_sensor_ctx_t *)func->ctx;
    ctx->raw_value = 0;
    ctx->value = 0.0f;
}

static void
tvoc_sensor_destroy(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
}

static const s_zb_attr_read_t*
tvoc_sensor_get_read_attrs(s_zb_device_t *device, s_zb_function_t *func)
{
    return s_tvoc_sensor_read_attrs;
}

static void
tvoc_sensor_notify_state(s_zb_device_t *device, s_zb_function_t *func, s_zb_device_tvoc_sensor_ctx_t *ctx)
{
...
static void
tvoc_sensor_on_attr_report(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    s_zb_device_tvoc_sensor_ctx_t *ctx = (s_zb_device_tvoc_sensor_ctx_t *)func->ctx;
    bool changed = false;

    if (msg->msg->cluster_id == ZCL_CLUSTER_ID_MS_TVOC_MEASUREMENT)
    {
        ...
        for (uint8_t i = 0; i < report_attr_cmd->num_attr; i++)
        {
            ...
            if (report_attr_info->attr_id == ATTRID_CONCENTRATION_MEASUREMENT_MEASURED_VALUE)
            {
                ...
                {
                    ZB_LOGW(TAG, "0x%llx - %s() - Unexpected TVOC data type: 0x%02x", device->ieee_addr, __func__, report_attr_info->data_type);
                    continue;
                }
                ...
                if (ctx->raw_value != raw_value)
                {
                    ...
                    ZB_LOGI(TAG, "0x%llx - %s() - TVOC: %.1f ppb", device->ieee_addr, __func__, ctx->value);
                    changed = true;
                }
            }
            else
    ...
    if (changed)
    {
        tvoc_sensor_notify_state(device, func, ctx);
    }
}


static void
tvoc_sensor_on_read_rsp(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    s_zb_device_tvoc_sensor_ctx_t *ctx = (s_zb_device_tvoc_sensor_ctx_t *)func->ctx;
    bool changed = false;
    ...
    if (msg->msg->cluster_id == ZCL_CLUSTER_ID_MS_TVOC_MEASUREMENT)
    {
        ...
        for (uint8_t i = 0; i < read_attr_rsp_cmd->num_attr; i++)
        {
            ...
            if (read_attr_rsp_info->attr_id == ATTRID_CONCENTRATION_MEASUREMENT_MEASURED_VALUE && read_attr_rsp_info->status == ZCL_STATUS_SUCCESS)
            {
                ...
                {
                    ZB_LOGW(TAG, "0x%llx - %s() - Unexpected TVOC data type: 0x%02x", device->ieee_addr, __func__, read_attr_rsp_info->data_type);
                    continue;
                }
    ...
    if (changed)
    {
        tvoc_sensor_notify_state(device, func, (s_zb_device_tvoc_sensor_ctx_t *)func->ctx);
    }
}

static zb_status_t
tvoc_sensor_on_command(s_zb_device_t *device, s_zb_function_t *func, const s_zb_cmd_t *cmd)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    ...
    switch (cmd->type)
    {
        case ZB_CMD_READ_STATE:
        {
            ...
            status = zb_zcl_send_read(ZB_HUB_ENDPOINT, &addr, ZCL_CLUSTER_ID_MS_TVOC_MEASUREMENT, read_attr_cmd, ZCL_FRAME_CLIENT_SERVER_DIR, false, seq_num);
            break;
        }
        default:
...
static void
tvoc_sensor_emit_state(s_zb_device_t *device, s_zb_function_t *func)
{
    tvoc_sensor_notify_state(device, func, (s_zb_device_tvoc_sensor_ctx_t *)func->ctx);
}

s_zb_function_ops_t zb_device_tvoc_sensor_ops = {
    .emit_state = tvoc_sensor_emit_state,
    .ctx_size = sizeof(s_zb_device_tvoc_sensor_ctx_t),
    .init = tvoc_sensor_init,
    .destroy = tvoc_sensor_destroy,
    .get_read_attrs = tvoc_sensor_get_read_attrs,
    .on_attr_report = tvoc_sensor_on_attr_report,
    .on_read_rsp = tvoc_sensor_on_read_rsp,
    .on_command = tvoc_sensor_on_command,
};
