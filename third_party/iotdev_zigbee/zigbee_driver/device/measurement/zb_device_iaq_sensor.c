...
static const s_zb_attr_read_t s_iaq_sensor_read_attrs[] = {
    { ZCL_CLUSTER_ID_MS_IAQ_MEASUREMENT, 1, { ATTRID_CONCENTRATION_MEASUREMENT_MEASURED_VALUE } },
};

static void
iaq_sensor_init(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    s_zb_device_iaq_sensor_ctx_t *ctx = (s_zb_device_iaq_sensor_ctx_t *)func->ctx;
    ctx->raw_value = 0;
    ctx->value = 0.0f;
}

static void
iaq_sensor_destroy(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
}

static const s_zb_attr_read_t*
iaq_sensor_get_read_attrs(s_zb_device_t *device, s_zb_function_t *func)
{
    return s_iaq_sensor_read_attrs;
}

static void
iaq_sensor_notify_state(s_zb_device_t *device, s_zb_function_t *func, s_zb_device_iaq_sensor_ctx_t *ctx)
{
    ZB_LOGD(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    s_zb_event_t event;
    event.type = ZB_EVENT_SENSOR_IAQ;
...
static bool
iaq_sensor_on_measured_value(s_zb_device_t *device, s_zb_function_t *func,
                             uint8_t data_type, const uint8_t *data)
{
    ...
    {
        raw_value = BUILD_UINT16(data[0], data[1]);
        value = (float)raw_value;
    }
    else
    {
        ZB_LOGW(TAG, "0x%llx - %s() - Unexpected IAQ data type: 0x%02x", device->ieee_addr, __func__, data_type);
        return false;
    }
    ...
    ctx->value = value;
    ZB_LOGI(TAG, "0x%llx - %s() - IAQ: %.1f", device->ieee_addr, __func__, ctx->value);
    return true;
}

static void
iaq_sensor_on_attr_report(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    bool changed = false;

    if (msg->msg->cluster_id == ZCL_CLUSTER_ID_MS_IAQ_MEASUREMENT)
    {
        ...
        for (uint8_t i = 0; i < report_attr_cmd->num_attr; i++)
        {
            ...
            {
                changed |= iaq_sensor_on_measured_value(device, func, report_attr_info->data_type, report_attr_info->attr_data);
            }
            else
            {
    ...
    if (changed)
    {
        iaq_sensor_notify_state(device, func, (s_zb_device_iaq_sensor_ctx_t *)func->ctx);
    }
}

static void
iaq_sensor_on_read_rsp(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    bool changed = false;
    ...
    if (msg->msg->cluster_id == ZCL_CLUSTER_ID_MS_IAQ_MEASUREMENT)
    {
        ...
        for (uint8_t i = 0; i < read_attr_rsp_cmd->num_attr; i++)
        {
            ...
            {
                changed |= iaq_sensor_on_measured_value(device, func, read_attr_rsp_info->data_type, read_attr_rsp_info->data);
            }
            else
            {
...
static zb_status_t
iaq_sensor_on_command(s_zb_device_t *device, s_zb_function_t *func, const s_zb_cmd_t *cmd)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    ...
    switch (cmd->type)
    {
        case ZB_CMD_READ_STATE:
        {
            ...
            status = zb_zcl_send_read(ZB_HUB_ENDPOINT, &addr, ZCL_CLUSTER_ID_MS_IAQ_MEASUREMENT, read_attr_cmd, ZCL_FRAME_CLIENT_SERVER_DIR, false, seq_num);
            break;
        }
        default:
...
static void
iaq_sensor_emit_state(s_zb_device_t *device, s_zb_function_t *func)
{
    iaq_sensor_notify_state(device, func, (s_zb_device_iaq_sensor_ctx_t *)func->ctx);
}

s_zb_function_ops_t zb_device_iaq_sensor_ops = {
    .emit_state = iaq_sensor_emit_state,
    .ctx_size = sizeof(s_zb_device_iaq_sensor_ctx_t),
    .init = iaq_sensor_init,
    .destroy = iaq_sensor_destroy,
    .get_read_attrs = iaq_sensor_get_read_attrs,
    .on_attr_report = iaq_sensor_on_attr_report,
    .on_read_rsp = iaq_sensor_on_read_rsp,
    .on_command = iaq_sensor_on_command,
};
