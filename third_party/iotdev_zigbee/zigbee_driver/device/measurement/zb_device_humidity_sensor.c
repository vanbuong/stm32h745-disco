...
static const s_zb_attr_read_t s_humidity_sensor_read_attrs[] = {
    { ZCL_CLUSTER_ID_MS_RELATIVE_HUMIDITY, 1, { ATTRID_RELATIVITY_HUMIDITY_MEASURED_VALUE } },
};

static void
humidity_sensor_init(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    s_zb_device_humidity_sensor_ctx_t *ctx = (s_zb_device_humidity_sensor_ctx_t *)func->ctx;
    ctx->raw_value = 0;
    ctx->value = 0.0f;
}

static void
humidity_sensor_destroy(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
}

static const s_zb_attr_read_t*
humidity_sensor_get_read_attrs(s_zb_device_t *device, s_zb_function_t *func)
{
    return s_humidity_sensor_read_attrs;
}

static void
humidity_sensor_notify_state(s_zb_device_t *device, s_zb_function_t *func, s_zb_device_humidity_sensor_ctx_t *ctx)
{
    ZB_LOGD(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    s_zb_event_t event;
    event.type = ZB_EVENT_SENSOR_HUMIDITY;
...
static void
humidity_sensor_on_attr_report(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    s_zb_device_humidity_sensor_ctx_t *ctx = (s_zb_device_humidity_sensor_ctx_t *)func->ctx;
    bool changed = false;

    if (msg->msg->cluster_id == ZCL_CLUSTER_ID_MS_RELATIVE_HUMIDITY)
    {
        ...
        for (uint8_t i = 0; i < report_attr_cmd->num_attr; i++)
        {
            ...
            if (report_attr_info->attr_id == ATTRID_RELATIVITY_HUMIDITY_MEASURED_VALUE)
            {
    ...
    if (changed)
    {
        humidity_sensor_notify_state(device, func, ctx);
    }
}

static void
humidity_sensor_on_read_rsp(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    s_zb_device_humidity_sensor_ctx_t *ctx = (s_zb_device_humidity_sensor_ctx_t *)func->ctx;
    bool changed = false;
    ...
    if (msg->msg->cluster_id == ZCL_CLUSTER_ID_MS_RELATIVE_HUMIDITY)
    {
        ...
        for (uint8_t i = 0; i < read_attr_rsp_cmd->num_attr; i++)
        {
            ...
            if (read_attr_rsp_info->attr_id == ATTRID_RELATIVITY_HUMIDITY_MEASURED_VALUE && read_attr_rsp_info->status == ZCL_STATUS_SUCCESS)
            {
...
static zb_status_t
humidity_sensor_on_command(s_zb_device_t *device, s_zb_function_t *func, const s_zb_cmd_t *cmd)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    ...
    switch (cmd->type)
    {
        case ZB_CMD_READ_STATE:
        {
            ...
            read_attr_cmd->attr_id[0] = ATTRID_RELATIVITY_HUMIDITY_MEASURED_VALUE;
            status = zb_zcl_send_read(ZB_HUB_ENDPOINT, &addr, ZCL_CLUSTER_ID_MS_RELATIVE_HUMIDITY, read_attr_cmd, ZCL_FRAME_CLIENT_SERVER_DIR, false, seq_num);
...
static void
humidity_sensor_emit_state(s_zb_device_t *device, s_zb_function_t *func)
{
    humidity_sensor_notify_state(device, func, (s_zb_device_humidity_sensor_ctx_t *)func->ctx);
}

s_zb_function_ops_t zb_device_humidity_sensor_ops = {
    .emit_state = humidity_sensor_emit_state,
    .ctx_size = sizeof(s_zb_device_humidity_sensor_ctx_t),
    .init = humidity_sensor_init,
    .destroy = humidity_sensor_destroy,
    .get_read_attrs = humidity_sensor_get_read_attrs,
    .on_attr_report = humidity_sensor_on_attr_report,
    .on_read_rsp = humidity_sensor_on_read_rsp,
    .on_command = humidity_sensor_on_command,
};