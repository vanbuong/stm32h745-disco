...
static const s_zb_attr_read_t s_mains_power_outlet_read_attrs[] = {
    { ZCL_CLUSTER_ID_GENERAL_ON_OFF, 1, { ATTRID_ON_OFF_ON_OFF } },
};

static void
mains_power_outlet_init(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    s_zb_device_mains_power_outlet_ctx_t *ctx = (s_zb_device_mains_power_outlet_ctx_t *)func->ctx;
    ctx->on_off = false;
}

static void
mains_power_outlet_destroy(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
}

static const s_zb_attr_read_t*
mains_power_outlet_get_read_attrs(s_zb_device_t *device, s_zb_function_t *func)
{
    (void)device;
    (void)func;
    return s_mains_power_outlet_read_attrs;
}

static void
mains_power_outlet_notify_state(s_zb_device_t *device, s_zb_function_t *func, s_zb_device_mains_power_outlet_ctx_t *ctx)
{
    ZB_LOGD(TAG, "0x%llx - %s()", device->ieee_addr, __func__);

    s_zb_event_t event = {0};
    event.type = ZB_EVENT_LIGHT_ONOFF_STATE;
    event.onoff_light.on = ctx->on_off;
    zb_device_manager_notify_event(device, func, &event);
}

static void
mains_power_outlet_on_event(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    s_zb_device_mains_power_outlet_ctx_t *ctx = (s_zb_device_mains_power_outlet_ctx_t *)func->ctx;
    bool changed = false;
    ...
    if (msg->msg->cluster_id == ZCL_CLUSTER_ID_GENERAL_ON_OFF)
    {
        if (msg->hdr.command_id == ZCL_CMD_REPORT)
        {
            ...
            for (uint8_t i = 0; i < report_attr_cmd->num_attr; i++)
            {
                ...
                {
                    bool on = (report_attr_info->attr_data[0] != 0);
                    if (ctx->on_off != on)
                    {
                        ZB_LOGI(TAG, "0x%llx - %s() - ON: (%d) -> (%d)", device->ieee_addr, __func__, ctx->on_off, on);
                        changed = true;
                    }
                    ctx->on_off = on;
                    break;
        ...
        else if (msg->hdr.command_id == ZCL_CMD_READ_RSP)
        {
            ...
            for (uint8_t i = 0; i < read_attr_rsp_cmd->num_attr; i++)
            {
                ...
                {
                    bool on = (read_attr_rsp_info->data[0] != 0);
                    if (ctx->on_off != on)
                    {
                        ZB_LOGI(TAG, "0x%llx - %s() - ON: (%d) -> (%d)", device->ieee_addr, __func__, ctx->on_off, on);
                        changed = true;
    ...
    if (changed)
    {
        mains_power_outlet_notify_state(device, func, ctx);
    }
}

static void
mains_power_outlet_on_attr_report(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    mains_power_outlet_on_event(device, func, msg);
}

static void
mains_power_outlet_on_read_rsp(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    mains_power_outlet_on_event(device, func, msg);
}
...
static zb_status_t
mains_power_outlet_on_command(s_zb_device_t *device, s_zb_function_t *func, const s_zb_cmd_t *cmd)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    ...
    switch (cmd->type)
    {
        case ZB_CMD_SET_ONOFF:
        {
            ...
            {
                s_zb_device_mains_power_outlet_ctx_t *ctx = (s_zb_device_mains_power_outlet_ctx_t *)func->ctx;
                if (ctx->on_off != cmd->onoff.on)
                {
                    ctx->on_off = cmd->onoff.on;
                    mains_power_outlet_notify_state(device, func, ctx);
                }
            }
            break;
...
static void
mains_power_outlet_emit_state(s_zb_device_t *device, s_zb_function_t *func)
{
    mains_power_outlet_notify_state(device, func, (s_zb_device_mains_power_outlet_ctx_t *)func->ctx);
}

s_zb_function_ops_t zb_device_mains_power_outlet_ops = {
    .emit_state = mains_power_outlet_emit_state,
    .ctx_size = sizeof(s_zb_device_mains_power_outlet_ctx_t),
    .init = mains_power_outlet_init,
    .destroy = mains_power_outlet_destroy,
    .get_read_attrs = mains_power_outlet_get_read_attrs,
    .on_attr_report = mains_power_outlet_on_attr_report,
    .on_read_rsp = mains_power_outlet_on_read_rsp,
    .on_command = mains_power_outlet_on_command,
};
