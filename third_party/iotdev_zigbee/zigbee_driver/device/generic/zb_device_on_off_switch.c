...
static void
on_off_switch_init(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    s_zb_device_on_off_switch_ctx_t *ctx = (s_zb_device_on_off_switch_ctx_t *)func->ctx;
    ctx->on_off = false;
}

static void
on_off_switch_destroy(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
}

static const s_zb_attr_read_t*
on_off_switch_get_read_attrs(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    return NULL;
}
...
static void
on_off_switch_notify_state(s_zb_device_t *device, s_zb_function_t *func, s_zb_device_on_off_switch_ctx_t *ctx)
{
    ZB_LOGD(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    s_zb_event_t event = {0};
    event.type = ZB_EVENT_SWITCH_ONOFF_STATE;
    event.switch_onoff.on = (ctx->on_off != 0);
    zb_device_manager_notify_event(device, func, &event);
}

static void
on_off_switch_on_read_rsp(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
}

static void
on_off_switch_on_attr_report(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    s_zb_device_on_off_switch_ctx_t *ctx = (s_zb_device_on_off_switch_ctx_t *)func->ctx;
    bool changed = false;
    ...
    if (changed)
    {
        on_off_switch_notify_state(device, func, (s_zb_device_on_off_switch_ctx_t *)func->ctx);
    }
}

static zb_status_t
on_off_switch_on_command(s_zb_device_t *device, s_zb_function_t *func, const s_zb_cmd_t *cmd)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
...
static void
on_off_switch_emit_state(s_zb_device_t *device, s_zb_function_t *func)
{
    on_off_switch_notify_state(device, func, (s_zb_device_on_off_switch_ctx_t *)func->ctx);
}

s_zb_function_ops_t zb_device_on_off_switch_ops = {
    .emit_state = on_off_switch_emit_state,
    .ctx_size = sizeof(s_zb_device_on_off_switch_ctx_t),
    .init = on_off_switch_init,
    .destroy = on_off_switch_destroy,
    .get_read_attrs = on_off_switch_get_read_attrs,
    .on_read_rsp = on_off_switch_on_read_rsp,
    .on_attr_report = on_off_switch_on_attr_report,
    .on_command = on_off_switch_on_command,
};