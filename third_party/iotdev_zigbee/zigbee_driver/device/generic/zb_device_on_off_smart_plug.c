...
static const s_zb_attr_read_t s_on_off_smart_plug_read_attrs[] = {
    { ZCL_CLUSTER_ID_GENERAL_ON_OFF, 1, { ATTRID_ON_OFF_ON_OFF } },
};

static void
on_off_smart_plug_init(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
}

static void
on_off_smart_plug_destroy(s_zb_device_t *device, s_zb_function_t *func)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
}

static const s_zb_attr_read_t*
on_off_smart_plug_get_read_attrs(s_zb_device_t *device, s_zb_function_t *func)
{
    (void)device;
    (void)func;
    return s_on_off_smart_plug_read_attrs;
}

static void
on_off_smart_plug_notify_state(s_zb_device_t *device, s_zb_function_t *func, s_zb_device_on_off_smart_plug_ctx_t *ctx)
{
...
static void
on_off_smart_plug_on_event(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    s_zb_device_on_off_smart_plug_ctx_t *ctx = (s_zb_device_on_off_smart_plug_ctx_t *)func->ctx;
    bool changed = false;
        ...
        if (msg->hdr.command_id == ZCL_CMD_REPORT)
        {
            ...
            for (uint8_t i = 0; i < report_attr_cmd->num_attr; i++)
            {
                ...
                if (report_attr_info->attr_id == ATTRID_ON_OFF_ON_OFF)
                {
                    ...
                    {
                        ZB_LOGI(TAG, "0x%llx - %s() - ON: (%d) -> (%d)", device->ieee_addr, __func__, ctx->on_off, report_attr_info->attr_data[0]);
                        changed = true;
                    }
        ...
        else if (msg->hdr.command_id == ZCL_CMD_READ_RSP)
        {
            ...
            for (uint8_t i = 0; i < read_attr_rsp_cmd->num_attr; i++)
            {
                ...
                if (read_attr_rsp_info->attr_id == ATTRID_ON_OFF_ON_OFF && read_attr_rsp_info->status == ZCL_STATUS_SUCCESS)
                {
                    ...
                    {
                        ZB_LOGI(TAG, "0x%llx - %s() - ON: (%d) -> (%d)", device->ieee_addr, __func__, ctx->on_off, read_attr_rsp_info->data[0]);
                        changed = true;
                    }
    ...
    if (changed)
    {
        on_off_smart_plug_notify_state(device, func, ctx);
    }
}

static void
on_off_smart_plug_on_attr_report(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    on_off_smart_plug_on_event(device, func, msg);
}

static void
on_off_smart_plug_on_read_rsp(s_zb_device_t *device, s_zb_function_t *func, s_zb_zcl_incoming_msg_t *msg)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
    on_off_smart_plug_on_event(device, func, msg);
}
...
static zb_status_t
on_off_smart_plug_on_command(s_zb_device_t *device, s_zb_function_t *func, const s_zb_cmd_t *cmd)
{
    ZB_LOGI(TAG, "0x%llx - %s()", device->ieee_addr, __func__);
...
static void
on_off_smart_plug_emit_state(s_zb_device_t *device, s_zb_function_t *func)
{
    on_off_smart_plug_notify_state(device, func, (s_zb_device_on_off_smart_plug_ctx_t *)func->ctx);
}

s_zb_function_ops_t zb_device_on_off_smart_plug_ops = {
    .emit_state = on_off_smart_plug_emit_state,
    .ctx_size = sizeof(s_zb_device_on_off_smart_plug_ctx_t),
    .init = on_off_smart_plug_init,
    .destroy = on_off_smart_plug_destroy,
    .get_read_attrs = on_off_smart_plug_get_read_attrs,
    .on_attr_report = on_off_smart_plug_on_attr_report,
    .on_read_rsp = on_off_smart_plug_on_read_rsp,
    .on_command = on_off_smart_plug_on_command,
};
